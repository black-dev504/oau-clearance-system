                ...
                <div class="hidden group-hover:flex items-center gap-3 shrink-0">
                    <div class="flex items-center gap-3 shrink-0">
                        @if($unit->status == 'deactivated')
                            <button wire:click="updateStatus({{$unit->id}}, 'activate')" class="border px-3 py-1.5 text-sm rounded-full bg-green-300">Activate</button>
                        @else
                            <button wire:click="updateStatus({{$unit->id}}, 'suspend')" class="border px-3 py-1.5 text-sm text-white rounded-full bg-red-400">Suspend</button>
