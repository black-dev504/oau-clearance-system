...
<flux:modal name="add-faculty" x-on:close="$wire.resetModal()" wire:key="add-faculty" class="min-w-2xl rounded-2xl !p-0" xmlns:flux="http://www.w3.org/1999/html">
    ...
    <div class="grid grid-cols-2 gap-4 border-t dark:border-white/10  border-gray-200 p-6">
        <button type="button" wire:click="resetModal"
                class="px-13 py-2 bg-white dark:bg-zinc-800 dark:text-zinc-400 dark:border-white/10 border border-gray-200 text-gray-700 rounded-[10px]" data-tw-dismiss="modal">
            Cancel
        </button>
        ...
        @if(!$this->editing)
            <flux:button variant="primary" color="violet"
                         ...
                         wire:target="addFaculty"
                         class="px-13 py-3 !bg-primary text-white  rounded-[10px]">

                <span wire:loading.remove wire:target="addFaculty">+ Add</span>
                <span wire:loading wire:target="addFaculty">Adding...</span>
            </flux:button>
        @else
            <flux:button variant="primary" color="violet"
                         wire:click="updateFaculty"
                         wire:loading.attr="disabled"
                         wire:target="updateFaculty"
                         class="px-13 py-3 !bg-primary text-white  rounded-[10px]">

                <span wire:loading.remove wire:target="updateFaculty">Update</span>
                <span wire:loading wire:target="updateFaculty">Updating...</span>
            </flux:button>
        @endif
